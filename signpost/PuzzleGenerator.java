...
class PuzzleGenerator implements PuzzleSource {
    ...
    /** Return an array representing WIDTH x HEIGHT Signpost puzzle.
    ...
    private int makeBackwardConnections(Model model) {
        ArrayList<Sq> conns = new ArrayList<>();
        int res;
        res = 1;
        Sq found;
        int foundn;
        ...
        for (Sq sq : model) {
            found = null;
            foundn = 0;
            if (sq.sequenceNum() != 0 && sq.predecessor() == null) {
                for (Place location: sq.successors()) {
                    Sq prediction = model.get(location);
                    if (prediction.connectable(sq)) {
                        found = prediction;
                        conns.add(prediction);
                        foundn++;
                    }
                }
                if (foundn == 0) {
    ...
    private int makeForwardConnections(Model model) {
        ...
        for (Sq sq : model) {
            found = null;
            foundn = 0;
            if (sq.direction() != 0 && sq.successor() == null) {
                for (Place location: sq.successors()) {
                    Sq squaresucc = model.get(location);
                    if (sq.connectable(squaresucc)) {
                        found = squaresucc;
                        conns.add(squaresucc);
                        foundn++;
                    }
                }
                if (foundn == 0) {
